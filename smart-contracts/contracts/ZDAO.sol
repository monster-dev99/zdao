...
contract ZDAO is ZamaEthereumConfig {
    using FHE for *;
    ...
    function submitDecryptedVoteCounts(
        uint256 proposalId,
        bytes memory abiEncodedClearVoteCounts,
        bytes memory decryptionProof
    ) external {
        if (proposalId >= proposals.length) revert InvalidProposal();
        
        Proposal storage p = proposals[proposalId];
        if (!p.isPublic) revert InvalidProposal();
        
        bytes32[] memory cts = new bytes32[](2);
        cts[0] = FHE.toBytes32(p.yesCount);
        cts[1] = FHE.toBytes32(p.noCount);
        
        FHE.checkSignatures(cts, abiEncodedClearVoteCounts, decryptionProof);
        
        (uint8 yesCount, uint8 noCount) = abi.decode(abiEncodedClearVoteCounts, (uint8, uint8));
        p.publicYesCount = yesCount;
        p.publicNoCount = noCount;
    }
}
