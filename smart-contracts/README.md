...
A decentralized autonomous organization built with Solidity and Fully Homomorphic Encryption (FHE) using the FHEVM framework.
...
\`\`\`javascript
await zdao.createProposal("Should we implement FHE voting?");
\`\`\`
...
await zdao.vote(proposalId, encryptedVote, proof);
\`\`\`
