...
async function main() {
  ...
  // Deploy the contract
  console.log("1. Deploying ZDAO contract...");
  ...
  for (let i = 0; i < proposals.length; i++) {
    const tx = await zdao.createProposal(proposals[i]);
    await tx.wait();
    console.log(`Created proposal ${i}: ${proposals[i]}`);
  }

  console.log("Total proposals:", await zdao.proposalCount());
  console.log("");

  // Get proposal information
  console.log("3. Proposal information:");
  for (let i = 0; i < proposals.length; i++) {
    const proposal = await zdao.proposals(i);
  ...
  for (let i = 0; i < proposals.length; i++) {
    for (let j = 0; j < 5; j++) {
      ...
      const hasVoted = await zdao.hasUserVoted(i, voter.address);
  ...
  for (let i = 0; i < proposals.length; i++) {
    const [yesCount, noCount] = await zdao.getEncryptedVoteCount(i);
  ...
  for (let i = 0; i < proposals.length; i++) {
    const tx = await zdao.makeVoteCountsPublic(i);
    await tx.wait();
  ...
  for (let i = 0; i < proposals.length; i++) {
    const [yesCount, noCount, isPublic] = await zdao.getPublicVoteCounts(i);
